...
def upload_pdf(request):
    ...
    if request.method == 'POST' and request.POST.get('job', ''):
        ...
        pdf_text=''
        job_text = ''
or    return [ent,job_text,pdf_text]    
...
def match_CJ(cv, job, lt):
    # Ensure that job contains meaningful content
    if not job:
        return [0, 0, [], []]
    ...
    ct = 0
    for s in lt[-1]:
        if s in process_resume(job):
            ct += 1

    miss_sk = []
    skills_job = []
    skills_nlp=get_entities(pdf_text, nlp, GM)[-1]
    for si in skills:
        si = si.lower()
        if si in process_resume(job):
            skills_job.append(si)
            if si not in lt[-1]:
                miss_sk.append(si)

    return [Match, ct, miss_sk, skills_job,skills_nlp]
