...
def upload_pdfs(request):
    if request.method == 'POST':
        ...
        for uploaded_file in uploaded_files:
            saved_file = fs.save(os.path.join(folder_path, uploaded_file.name), uploaded_file)
            pdf_path = os.path.join(folder_path, saved_file)
            pdf_text = extract_text_from_pdf(pdf_path)
            pdf_texts.append(pdf_text)

        return render(request, 'base.html', {'pdf_texts': pdf_text})
