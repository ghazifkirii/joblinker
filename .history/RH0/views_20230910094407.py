...
def job_desc(request):
    if request.method == 'POST':
        job_text = request.POST.get('job', '')  # 'job' is the name attribute of the input field
        # Now you can use the 'job_text' variable in your backend logic

        # For example, you can return a response with the entered text
        return HttpResponse(f'You entered: {job_text}')
    else:
        # Handle GET request or render an initial form
        return render(request, 'upload_pdf.html')